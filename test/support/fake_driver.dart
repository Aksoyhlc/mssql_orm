...
mixin _RecordsCalls {
  ...
  Object? takeFailure() {
    if (_failureIndex >= failures.length) return null;
    return failures[_failureIndex++];
  }
...
class FakeConnection with _RecordsCalls implements MssqlConnection {
  ...
  dynamic noSuchMethod(Invocation invocation) {
    ...
    switch (member) {
      case 'queryRows':
        record(invocation);
        final queryRowsFailure = takeFailure();
        if (queryRowsFailure != null) {
          return Future<List<Map<String, Object?>>>.error(queryRowsFailure);
        }
        return Future<List<Map<String, Object?>>>.value(nextReply());
      case 'query':
        record(invocation);
        final queryFailure = takeFailure();
        if (queryFailure != null) {
          return Future<MssqlExecutionResult>.error(queryFailure);
        }
        return Future<MssqlExecutionResult>.value(driverResult(nextReply()));
      case 'queryTypedRows':
        record(invocation);
        final typedFailure = takeFailure();
        if (typedFailure != null) {
          return Future<List<MssqlRow>>.error(typedFailure);
        }
        return Future<List<MssqlRow>>.value(driverRows(nextReply()));
      case 'execute':
        record(invocation);
        final executeFailure = takeFailure();
        if (executeFailure != null) {
          return Future<int>.error(executeFailure);
        }
...
class FakeTransaction with _RecordsCalls implements MssqlTransaction {
  FakeTransaction({
    MssqlDecimalMode decimalMode = MssqlDecimalMode.doublePrecision,
  }) : connection = FakeConnection(decimalMode: decimalMode);
  ...
  @override
  String get currentDatabase => connection.config.database;
  ...
  dynamic noSuchMethod(Invocation invocation) {
    ...
    switch (member) {
      case 'savepoint':
        savepointCalls++;
        final callback = invocation.positionalArguments.single as Function;
        return Function.apply(callback, const <Object?>[]);
      case 'query':
        record(invocation);
        final queryFailure = takeFailure();
        if (queryFailure != null) {
      ...
      default:
        throw UnimplementedError(
          'FakeTransaction was asked for "$member", which no test has taught '
          'it to answer.',
        );
    }
  }
